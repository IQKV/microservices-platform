# Gripday Auth Service - Kubernetes Deployment

This directory contains Kubernetes manifests and deployment scripts for the Gripday Auth Service.
...
http://auth.pynity.site/swagger-ui/*      → Swagger UI
http://auth.pynity.site/actuator/*        → Health/metrics
```
...
5. **Service not accessible**
   ...
   kubectl describe ingress auth-service-ingress -n gripday-auth
   ```
...
# View configuration
kubectl describe configmap auth-service-config -n gripday-auth
```
...
curl -X POST http://auth.pynity.site/api/v1/auth/signup \
...
# Test user login
curl -X POST http://auth.pynity.site/api/v1/auth/login \
  -H "Content-Type: application/json" \
  -d '{
    "username": "testuser",
    "password": "TestPass123!"
...
curl -H "Authorization: Bearer $TOKEN" \
     http://auth.pynity.site/api/v1/users/me
```
