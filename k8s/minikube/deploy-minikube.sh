...
if minikube addons list | grep -q "ingress.*enabled"; then
    ...
    ok "Ingress applied"
    log "Add to /etc/hosts: $MINIKUBE_IP api.gripday.site user.gripday.site bookstore.gripday.site"
else
...
if [ "$WAIT_FOR_READY" = true ]; then
    ...
    kubectl wait --for=condition=ready pod -l app=postgres-user -n gripday-dev-env --timeout=300s || warn "Postgres user not ready"
    kubectl wait --for=condition=ready pod -l app=postgres-bookstore -n gripday-dev-env --timeout=300s || warn "Postgres bookstore not ready"
    kubectl wait --for=condition=ready pod -l app=redis -n gripday-dev-env --timeout=300s || warn "Redis not ready"
    kubectl wait --for=condition=ready pod -l app=user-service -n gripday-dev-env --timeout=300s || warn "User service not ready"
    kubectl wait --for=condition=ready pod -l app=bookstore-service -n gripday-dev-env --timeout=300s || warn "Bookstore service not ready"
    kubectl wait --for=condition=ready pod -l app=gateway-service -n gripday-dev-env --timeout=300s || warn "Gateway service not ready"
    ok "All pods ready"
fi
...
if minikube addons list | grep -q "ingress.*enabled"; then
    ...
    echo "  User:        http://user.gripday.site"
    echo "  Bookstore:   http://bookstore.gripday.site"
    echo ""
fi
