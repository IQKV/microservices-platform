...
kubectl get namespace gripday-dev-env > /dev/null 2>&1 || { log "Namespace doesn't exist. Nothing to clean."; exit 0; }

log "Resources to delete:"
kubectl get all -n gripday-dev-env 2>/dev/null || echo "  None"
echo ""

if [ "$FORCE" != true ]; then
    read -p "Delete all resources in gripday-dev-env? (yes/no): " -r
...
timeout=60
counter=0
while kubectl get namespace gripday-dev-env > /dev/null 2>&1; do
