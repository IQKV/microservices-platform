...
# Environment variable loading and validation script for Gripday Gateway Service
...
usage() {
    ...
    echo "and validates required variables for the gateway service."
}
...
get_required_variables() {
    ...
    case "$env" in
        "local")
            echo "SPRING_PROFILES_ACTIVE GRIPDAY_CACHE_REDIS_HOST GRIPDAY_GATEWAY_ROUTING_AUTH_SERVICE_URI GRIPDAY_GATEWAY_SECURITY_JWT_SECRET GRIPDAY_GATEWAY_SECURITY_AUTH_SERVICE_URL GRIPDAY_GATEWAY_CORS_ALLOWED_ORIGINS"
            ;;
        "staging")
            echo "SPRING_PROFILES_ACTIVE GRIPDAY_CACHE_REDIS_HOST GRIPDAY_CACHE_REDIS_PASSWORD GRIPDAY_GATEWAY_ROUTING_AUTH_SERVICE_URI GRIPDAY_GATEWAY_SECURITY_JWT_SECRET GRIPDAY_GATEWAY_SECURITY_AUTH_SERVICE_URL GRIPDAY_GATEWAY_CORS_ALLOWED_ORIGINS JWT_SECRET REDIS_PASSWORD"
            ;;
        "production")
            echo "SPRING_PROFILES_ACTIVE GRIPDAY_CACHE_REDIS_HOST GRIPDAY_CACHE_REDIS_PASSWORD GRIPDAY_GATEWAY_ROUTING_AUTH_SERVICE_URI GRIPDAY_GATEWAY_SECURITY_JWT_SECRET GRIPDAY_GATEWAY_SECURITY_AUTH_SERVICE_URL GRIPDAY_GATEWAY_CORS_ALLOWED_ORIGINS REDIS_HOST JWT_SECRET ALLOWED_ORIGINS"
            ;;
    esac
}
...
validate_environment_config() {
    ...
    if [[ ! "$GRIPDAY_GATEWAY_ROUTING_AUTH_SERVICE_URI" =~ ^https?:// ]]; then
        log_error "Invalid auth service URL format: $GRIPDAY_GATEWAY_ROUTING_AUTH_SERVICE_URI"
        log_error "Expected format: http://host:port or https://host:port"
        exit 1
    fi
    
    # Validate CORS origins format
    if [[ "$GRIPDAY_GATEWAY_CORS_ALLOWED_ORIGINS" == *"localhost"* ]] && [ "$ENVIRONMENT" = "production" ]; then
        log_warning "CORS configuration includes localhost in production environment"
    fi
    
    # Validate JWT secret strength for production
    if [ "$ENVIRONMENT" = "production" ]; then
        if [ ${#GRIPDAY_GATEWAY_SECURITY_JWT_SECRET} -lt 32 ]; then
            log_error "JWT secret is too short for production (minimum 32 characters)"
            exit 1
        fi
        
        if [[ "$GRIPDAY_GATEWAY_SECURITY_JWT_SECRET" == *"development"* ]] || [[ "$GRIPDAY_GATEWAY_SECURITY_JWT_SECRET" == *"local"* ]]; then
...
display_configuration_summary() {
    ...
    if [ ${#cors_origins} -gt 50 ]; then
        cors_origins="${cors_origins:0:47}..."
    fi
...
# Main execution
main() {
    log_info "Loading Gripday Gateway Service environment: $ENVIRONMENT"
    echo
    ...
    echo
    log_info "To start the gateway service with this environment:"
