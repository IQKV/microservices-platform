...
REQUIRED="SPRING_PROFILES_ACTIVE GRIPDAY_CACHE_REDIS_HOST GRIPDAY_GATEWAY_ROUTING_USER_SERVICE_URI GRIPDAY_GATEWAY_SECURITY_JWT_SECRET"
for var in $REQUIRED; do
    [ -z "${!var:-}" ] && err "Missing required variable: $var"
done

[[ "$GRIPDAY_GATEWAY_ROUTING_USER_SERVICE_URI" =~ ^https?:// ]] || err "Invalid user service URL"

[ "$ENV" = "production" ] && [ ${#GRIPDAY_GATEWAY_SECURITY_JWT_SECRET} -lt 32 ] && err "JWT secret too short for production"
