...
# Health check script for Gripday Gateway Service
# Waits for the gateway service to be healthy and ready to accept requests
...
usage() {
    ...
    echo
    echo "Parameters:"
    echo "  HOST     - Gateway service host (default: $DEFAULT_HOST)"
    echo "  PORT     - Gateway service port (default: $DEFAULT_PORT)"
    ...
    echo "  $0 gateway-service 8080              # Custom host and port"
    echo "  $0 localhost 8080 60 2              # All parameters"
    echo
    echo "Environment Variables:"
    echo "  GATEWAY_SERVICE_HOST - Override default host"
    echo "  GATEWAY_SERVICE_PORT - Override default port"
}
...
HOST="${GATEWAY_SERVICE_HOST:-$HOST}"
PORT="${GATEWAY_SERVICE_PORT:-$PORT}"
...
wait_for_health() {
    log_info "Waiting for gateway service at $HOST:$PORT to be healthy..."
    ...
    while [ $(date +%s) -lt $end_time ]; do
        ...
        if curl -f -s -m 10 "$HEALTH_URL" > /dev/null 2>&1; then
            log_success "Gateway service is responding to health checks"
            ...
            if [ "$status" = "UP" ]; then
                log_success "Gateway service is healthy (status: UP)"
                ...
                if curl -f -s -m 5 "$READY_URL" > /dev/null 2>&1; then
                    ...
                    if [ "$ready_status" = "UP" ]; then
                        log_success "Gateway service is ready to accept requests"
                        return 0
                    else
                        log_info "Gateway service is healthy but not ready (readiness: $ready_status)"
                    fi
                else
                    log_success "Gateway service is healthy and ready"
                    return 0
                fi
            else
                log_info "Gateway service is responding but not healthy (status: $status)"
            fi
        else
            log_info "Attempt $attempts: Gateway service not responding yet..."
        fi
        
        sleep "$INTERVAL"
    done
    
    log_error "Timeout reached after ${TIMEOUT}s. Gateway service is not healthy."
    return 1
}
...
# Main execution
main() {
    log_info "Gripday Gateway Service Health Check"
    echo
