...
REQUIRED="SPRING_PROFILES_ACTIVE GRIPDAY_DATABASE_URL GRIPDAY_DATABASE_USERNAME GRIPDAY_DATABASE_PASSWORD GRIPDAY_CACHE_REDIS_HOST GRIPDAY_AUTH_JWT_SECRET"
for var in $REQUIRED; do
    [ -z "${!var:-}" ] && err "Missing required variable: $var"
done

[[ "$GRIPDAY_DATABASE_URL" =~ ^jdbc:postgresql:// ]] || err "Invalid database URL"

[ "$ENV" = "production" ] && [ ${#GRIPDAY_AUTH_JWT_SECRET} -lt 32 ] && err "JWT secret too short for production"
