...
public class VerificationMetrics {
  ...
  public VerificationMetrics(final MeterRegistry meterRegistry) {
    // Email sending metrics
    this.emailsSentCounter = Counter.builder("iqscaffold_user_email_verification_sent_total")
        .description("Total number of verification emails sent")
        .register(meterRegistry);

    this.emailsFailedCounter = Counter.builder("iqscaffold_user_email_verification_send_failed_total")
        .description("Total number of failed verification email sends")
        .register(meterRegistry);

    // Email verification metrics
    this.verificationsSuccessfulCounter = Counter.builder("iqscaffold_user_email_verification_success_total")
        .description("Total number of successful email verifications")
        .register(meterRegistry);

    this.verificationsFailedCounter = Counter.builder("iqscaffold_user_email_verification_failed_total")
        .description("Total number of failed email verification attempts")
        .register(meterRegistry);

    // Token cleanup metrics
    this.tokensCleanedUpCounter = Counter.builder("iqscaffold_user_email_verification_tokens_cleaned_total")
        .description("Total number of expired verification tokens cleaned up")
        .register(meterRegistry);

    // Timing metrics
    this.emailSendTimer = Timer.builder("iqscaffold_user_email_send_duration_seconds")
        .description("Time taken to send verification emails")
        .register(meterRegistry);

    this.cleanupTimer = Timer.builder("iqscaffold_user_email_verification_cleanup_duration_seconds")
    ...
    Gauge.builder("iqscaffold_user_email_verification_expired_tokens_current", expiredTokensGauge, AtomicLong::get)
